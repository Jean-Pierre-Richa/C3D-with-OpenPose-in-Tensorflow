...
def run_training():
  ...
  # with tf.graph().as_default():
    ...
    # 1
    # exclude = [ weights['out'],biases['out'] ]
    # vals = list((set(weights.values()) | set(biases.values())) - set(exclude))
    # print('vals: ', vals)
    # saver = tf.train.Saver(vals)

    # 2
    # all_vars = tf.trainable_variables()
    # var_to_restore = [v for v in all_vars if not (v.name.startswith('bout') or v.name.startswith('wout'))]
    # saver = tf.train.Saver(var_to_restore)

    # 3
    # all_vars = tf.trainable_variables()
    # wVals = [v for v in weights.values() if not v.name.startswith('var_name/wout')]
    # bVals = [v for v in biases.values() if not v.name.startswith('var_name/bout')]
